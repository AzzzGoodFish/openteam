...
3. **蒸馏（Distill）**：定期（默认 7 天或 20 条记忆）对全量记忆库做全局整理——合并重复、浓缩细节、删除无效内容。不会因为"与当前工作无关"而删除笔记，记忆跨项目、跨时期保留。也可通过 `openteam distill` 手动触发
...
由于系统会在对话结束后自动巩固记忆，以下工具主要用于**主动精细控制**：
