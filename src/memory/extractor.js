...
## summary 书写要求
summary 是 agent 判断"这条笔记是否与当前任务相关"的唯一依据（agent 看不到 content，只能看到 summary）。
因此 summary 必须信息丰富，包含关键事实和要点，100-150 字左右。用分号分隔多个要点。
- 差：「OpenTeam 整体架构设计」
- 好：「OpenTeam 插件架构：3 个 Hook（messagesTransform/systemTransform/event）注入消息标记、记忆和团队上下文；CLI 管理 serve 进程和多 agent 实例；运行时文件存储在 ~/.opencode/agents 下」

## 输出格式
用 yaml 代码块包裹输出。代码块前后可以有分析文字。
...
actions:
  - action: create|update|append|delete
    ...
    key: note-key
    reason: 简短说明（可选）
    summary: 信息丰富的摘要（100-150字，包含关键事实和要点）
    content: |
      详细内容
\`\`\`
...
## 删除约束（重要）
- **禁止因为"与当前工作无关"或"不在焦点上"而删除笔记**。记忆应该跨项目、跨时期保留。
- 只有以下情况才允许删除：
  1. 内容已完全合并到另一条笔记中（merge 后的源笔记）
  2. 内容明确过时且有误导性（如记录了已废弃的 API）
  3. 内容为空或无实质信息
- 如果不确定是否应该删除，选择 keep。
...
// 从模型输出中提取 YAML：代码块 > 整段解析 > actions: 截取
function extractYamlBlock(text) {
  ...
  if (fenceMatch) {
    try {
      const parsed = YAML.parse(fenceMatch[1]);
      if (parsed) return parsed;
    } catch {
      // 代码块内 YAML 无效，继续 fallback
    }
  }
  ...
  // 从 "actions:" 到文本结尾截取
  const idx = trimmed.indexOf('actions:');
  if (idx >= 0) {
    try {
      return YAML.parse(trimmed.slice(idx));
    } catch {
      // ignore
    }
  }
