...
## 输出格式
用 \`---YAML---\` 和 \`---END---\` 标记包裹 YAML 输出。标记前后可以有分析文字。
...
// 从模型输出中提取 YAML：优先 ---YAML---/---END--- 标记，fallback 到 actions: 截取
function extractYamlBlock(text) {
  ...
  // 优先提取 ---YAML--- ... ---END--- 标记块
  const markerMatch = cleaned.match(/---YAML---\s*\n([\s\S]*?)\n---END---/);
  if (markerMatch) {
    try {
      const parsed = YAML.parse(markerMatch[1]);
      if (parsed) return parsed;
    } catch {
      // 标记内 YAML 无效，继续 fallback
    }
  }
