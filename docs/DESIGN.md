...
当 agent 收到 `[from boss]` 消息时，说明老板亲自介入，通常意味着工作方向有偏差或理解有误。agent 应认真理解并执行 boss 的指示。记忆的更新由系统在对话结束后自动巩固，agent 无需主动操作记忆工具来响应 boss 消息。
...
| 蒸馏 | Distill | 距上次蒸馏 ≥ 7 天或条目数 ≥ 20 | 对全量记忆库做全局整理：合并重复、浓缩细节、删除无效内容 |

**蒸馏删除约束**：蒸馏不会因为"与当前工作无关"而删除笔记。记忆应跨项目、跨时期保留。只有内容已被合并、明确过时有误导、或内容为空时才允许删除。
